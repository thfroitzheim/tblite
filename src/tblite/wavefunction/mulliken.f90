...
subroutine get_mulliken_atomic_charges_gradient(bas, mol, smat, pmat, dsmat, dpmatdr, dpmatdL, dqatdr, dqatdL)
   ...
   real(wp), allocatable :: tmp(:,:), dpsmat(:,:), pdsmat(:,:)

   allocate(tmp(size(dqatdr,2), size(dqatdr,3)), dpsmat(bas%nao,bas%nao), pdsmat(bas%nao,bas%nao))
   ...
   do spin = 1, size(pmat, 3)
      do ic = 1, 3
         ...
         do iao = 1, bas%nao
            ...
            do jao = 1, bas%nao
               ! dpao =  & !dpao - 
               ! & (dsmat(ic, iao, jao) * pmat(jao, iao, spin) + smat(iao, jao) * dpmatdr(ic, jao, iao, spin))
               !& (dsmat(ic, iao, jao) * pmat(iao, jao, spin) + smat(iao, jao) * dpmatdr(ic, iao, jao, spin))

               !dqatdr(ic, bas%ao2at(iao), bas%ao2at(jao)) = dqatdr(ic, bas%ao2at(iao), bas%ao2at(jao)) &

               tmp(bas%ao2at(iao), bas%ao2at(jao)) = tmp(bas%ao2at(iao), bas%ao2at(jao)) & 
               & + (dsmat(ic, iao, jao) * pmat(iao, jao, spin) + smat(iao, jao) * dpmatdr(ic, iao, jao, spin))
               ! do kao = 1, bas%nao
               !    if(jao == kao) then
               !       dqatdr(ic, bas%ao2at(iao), bas%ao2at(kao)) = dqatdr(ic, bas%ao2at(iao), bas%ao2at(kao)) &
               !       & + (dsmat(ic, kao, iao) * pmat(kao, iao, spin) + smat(kao, iao) * dpmatdr(ic, kao, iao, spin))
               !    else
               !       dqatdr(ic, bas%ao2at(iao), bas%ao2at(kao)) = dqatdr(ic, bas%ao2at(iao), bas%ao2at(kao)) &
               !       & - (dsmat(ic, kao, iao) * pmat(kao, iao, spin) + smat(kao, iao) * dpmatdr(ic, kao, iao, spin))
               !    end if 
               ...
               !dqatdr(ic, bas%ao2at(iao), bas%ao2at(jao)) = dqatdr(ic, bas%ao2at(iao), bas%ao2at(jao)) + dpao
               !& (dsmat(ic, jao, iao) * pmat(jao, iao, spin) + smat(jao, iao) * dpmatdr(ic, jao, iao, spin))
               !dpao
               !if (iao /= jao) then
               !      dqatdr(ic, bas%ao2at(jao), bas%ao2at(iao)) = dqatdr(ic, bas%ao2at(jao), bas%ao2at(iao)) - dpao
               !      !& (dsmat(ic, jao, iao) * pmat(jao, iao, spin) + smat(jao, iao) * dpmatdr(ic, jao, iao, spin))
               !      !dpao
               !end if 
               
            end do
            !dqatdr(ic, bas%ao2at(iao), bas%ao2at(iao)) = dqatdr(ic, bas%ao2at(iao), bas%ao2at(iao)) + dpao
         end do
         ...
         ! call gemm(dpmatdr(ic, :, :, 1), smat, dpsmat)
         ! call gemm(pmat(:, :, 1), dsmat(ic,:,:), pdsmat)
         
      end do
   end do

   ! call updown_to_magnet(dqatdr)
